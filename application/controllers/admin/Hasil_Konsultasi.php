 ...
 class Hasil_Konsultasi extends CI_Controller {
    ...
    public function index()
    {
		...
		$data = array(
			...
			'isi' => 'c_admin/v_hasil_konsultasi'
		);
		...
		if ($validation->run() == FALSE){
			...
			redirect('admin/Hasil_Konsultasi');
		}
	}
	...
	public function detail($id = null)
    {
		...
		$data = array(
			...
			'isi' => 'c_admin/v_detail_konsultasi'
		);
		$this->load->view('templat/index',$data);
		...
		// 	$mdl->add($data);
		// 	$this->session->set_flashdata('success', 'Berhasil disimpan');
		// 	redirect('admin/Detail_Konsultasi');
		// }
	}
	...
	public function edit($ID_HASIL = NULL)
	{
		...
		if(!isset($ID_HASIL)) redirect('admin/Hasil_Konsultasi');
		$mdl = $this->M_hasil_konsultasi;
		$validation = $this->form_validation;
		$post = $this->input->post();
		$validation->set_rules($this->rules()); 
